  ...
  <div class="card">
      ...
      <div class="card-header">
          <h4 class="card-title aaa">
            Manage Size
             <a href="{{ route('size.create') }}" class="add_new_btn btn btn-sm btn-primary">
              ...
              <table id="zero_config" class="table table-bordered text-center">
                ...
                @foreach ($info as $item)
                  ...
                  <tr>
                    ...
                    <td>{{ $item->name?? 'NO Size Available' }}</td>
                    <td>
                      <label class="switch">
                       ...
                       <button title="Action" class="btn without-focus border-0 px-1 py-0 mr-2" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            <i class="fa fa-ellipsis-v"></i>
                              ...
                              <div class="dropdown-menu" role="menu" style="">
                                 ...
                                 <a class="dropdown-item" href="{{ route('size.edit',$item->id) }}">
                                  <i class="fa fa-edit"></i> Edit
                                </a>

                                 <form action="{{ route('size.delete',$item->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this?')">
                                    @csrf
...
$(document).ready(function() {
    $('.status-switch').on('change', function() {
        ...
        $.ajax({
            url: "{{ route('size.updateStatus') }}", // You'll need to create this route
            type: 'POST',
            data: {
                size_id: sizeId,
